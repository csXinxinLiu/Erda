...
void write_with_imm1(struct context *ctx){
	//printf("write with imm\n");
 	struct ibv_send_wr wr, *bad_wr = NULL;
	struct ibv_sge sge;
	memset( &wr,0,sizeof(wr));
	...
	wr.wr.rdma.remote_addr = (uintptr_t)keybuf_mr.addr+0x20;
	wr.wr.rdma.rkey = keybuf_mr.rkey;
 	...
 	struct context *ctx = (struct context *)(uintptr_t)wc->wr_id;
  	...
  	if (wc->status != IBV_WC_SUCCESS){
  	  ...
  	  if (wc->opcode & IBV_WC_RECV){ 
  		...
  		else if(msg_tmp.type == LOGCLEAN){
  			printf("LOGCLEAN\n");
            post_receives(ctx);
            c_tag = 1;
			return 0;
  		}
  		else if(msg_tmp.type == LOGCLEANFINISHED){
  			printf("LOGCLEANFINISHED\n");
            post_receives(ctx);
            c_tag = 0;
			memcpy((void*)(&(pointer_mr[msg_tmp.hid])), (void *)(&(msg_tmp.mr)), sizeof(struct ibv_mr));
			return 0;
  		}
   		/*else if(msg_tmp.type == INSETW){
   		    if(compute==1){
			    gettime8();
			    compute_writelatency();		    
            }                                                
		...
		rcount++;
		//printf("%lld\n%lld\n",((1000000*end2.tv_sec+end2.tv_nsec/1000)-(1000000*end.tv_sec+end.tv_nsec/1000)),((1000000*end2.tv_sec+end2.tv_nsec/1000)-(1000000*end3.tv_sec+end3.tv_nsec/1000))); 
		...
		if(x == 0){  
        	...
        	gettime4();
        	compute_readlatency();
        	//printf("READ DATA: data-%s read complete\n", ctx->buffer);
...
void load_workloads(struct context *ctx){
    FILE* workload_1;
    workload_1 = fopen("dataseti_load.txt", "r");
...
void runload_workloads(struct context *ctx){
    FILE* workload_1;
    workload_1 = fopen("dataseti_run.txt", "r");
...
int on_connection(struct rdma_cm_id *id)
{
    ...
    gettime();
    //sleep(1);
    long long c = ((1000000*end.tv_sec+end.tv_nsec/1000) - (1000000*start.tv_sec+start.tv_nsec/1000)-500*10000-200*10000);
    float t = (float)c/20000;
    printf("%lld,%.2f\n",c,t);
    //printf("%lld,%.2f\n",RTotalTime+WTotalTime,(float)(RTotalTime+WTotalTime)/20000);  
    //printf("%d,%d\n",wcount,rcount);
	printf("END\n");
