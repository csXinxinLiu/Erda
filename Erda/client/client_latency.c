...
struct context{
	...
	struct ibv_mr *recvreadbuffer_mr;
...
int read_remote(struct context *ctx, int len, uint64_t offset, int hid)
{
	...
	struct ibv_send_wr wr, *bad_wr = NULL;
	struct ibv_sge sge;
	memset(ctx->recvreadbuffer, 0, len);
	...
	if(len){
		wr.sg_list = &sge;
		wr.num_sge = 1;
		sge.addr = (uintptr_t)ctx->recvreadbuffer;
		sge.length = len;
		sge.lkey = ctx->recvreadbuffer_mr->lkey;
	}
 	TEST_NZ(ibv_post_send(ctx->qp, &wr, &bad_wr));
}
...
void register_memory(struct context *ctx)
{
	...
	ctx->recvreadbuffer = malloc(BUFFER_SIZE);
	...
	TEST_Z(ctx->buffer_mr = ibv_reg_mr(
	 	...
	 	BUFFER_SIZE,
	 	IBV_ACCESS_LOCAL_WRITE|IBV_ACCESS_REMOTE_WRITE|IBV_ACCESS_REMOTE_READ));
	TEST_Z(ctx->recvreadbuffer_mr = ibv_reg_mr(
	 	ctx->pd,
	 	ctx->recvreadbuffer,
	 	BUFFER_SIZE,
	 	IBV_ACCESS_LOCAL_WRITE|IBV_ACCESS_REMOTE_WRITE|IBV_ACCESS_REMOTE_READ));
...
int on_connection(struct rdma_cm_id *id)
{
    ...
    run_workloads(ctx);
    gettime();
    long long totaltime = ((1000000*end.tv_sec+end.tv_nsec/1000) - (1000000*start.tv_sec+start.tv_nsec/1000)-500*10000);
    printf("%lld,%.2f\n",totaltime,(float)totaltime/20000);
    sleep(1);    
