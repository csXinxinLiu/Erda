...
int main(int argc, char **argv)
{
  ...
  pthread_create(&listen_thread,NULL,hey_listen,listener);
...
void *hey_listen(void *cmid){
  ...
  struct sockaddr_in * addr_m = (struct sockaddr_in*)rdma_get_local_addr(cm_id);
  // printf("addr:%s\n",inet_ntoa(addr_m->sin_addr));
...
void build_context(struct ibv_context *verbs)
{
  ...
  pthread_detach(s_ctx->cq_poller_thread);
}
