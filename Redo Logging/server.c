  ...
  if (wc->status == IBV_WC_SUCCESS){
    if (wc->opcode & IBV_WC_RECV){
      ...
      if(msg_tmp.type == PUT){
        ...
        strcpy(RB->data[RB->pwrite],msg_tmp.data);
        pflush((uint64_t *)&(RB->data[RB->pwrite]),((valuesize+keysize-1)/64+1)*global_write_latency_ns);//+++++++++++++++++++++
        //asm_mfence();
        RB->pwrite = nextpwrite(RB->pwrite);
        (RB->size)++;

        struct Message msg;
        msg.type = WRITEBUFFEROK;
        send_msg(conn, &msg);
        ...
        struct Message msg;
        msg.type = REGRBUFOK;
        register_metadatamemory(conn,RBUFFERSIZE*(valuesize+20),(char *)RB->data);
