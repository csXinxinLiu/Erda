...
int read_remote(struct context *ctx, int len, uint64_t offset);
  	...
  	if (wc->status != IBV_WC_SUCCESS){
  	  ...
  	  if (wc->opcode & IBV_WC_RECV){ 
  		...
  		if(msg_tmp.type == RHEAD1){
  			...
  			read_remote(ctx,valuesize+20,0);
		...
		//printf("READ DATA: data-%s read success\n",ctx->hbuffer);
  		if(compute == 1){
  			gettime2();
            wcount++;
...
void register_memory(struct context *ctx)
{
    ...
    ctx->hbuffer = malloc(valuesize+28);
...
int on_connection(struct rdma_cm_id *id)
{
    ...
    gettime();
	sleep(1);
    long long totaltime = ((1000000*end.tv_sec+end.tv_nsec/1000) - (1000000*start.tv_sec+start.tv_nsec/1000)-500*10000-100*10000);
